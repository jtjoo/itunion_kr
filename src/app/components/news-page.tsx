              ...
              <button
                ...
                className={`relative px-3 py-1 text-[16px] font-['Noto_Sans_KR',sans-serif] transition-colors ${
                  isActive
                    ...
                    : "bg-background text-[#767676] hover:text-[#303030]"
                }`}
              >
                {tab.label}
                ...
                {!isActive && (
                  <span className="absolute inset-0 border-b border-t border-r border-border-muted pointer-events-none rounded-tr" />
                )}
              </button>
            );
          })}
          {/* Extend bottom border to fill remaining width */}
          <div className="flex-1 border-b border-border-muted" />
        </div>
      </div>
          ...
          <div className="max-w-[1200px] mx-auto px-4 sm:px-8 md:px-16 py-10 sm:py-12 md:py-16 flex flex-col md:flex-row gap-6 sm:gap-8 md:gap-12 items-start justify-center">
            ...
            <div className="flex flex-col gap-6 flex-1 md:w-[484px] md:min-w-[300px] md:h-[351px]">
              <div className="flex flex-col gap-2">
                <h2 className="font-['Noto_Sans_KR',sans-serif] text-[20px] md:text-[24px] text-foreground leading-[1.2] tracking-[-0.48px]">
                  {featured.title}
                </h2>
              ...
              <p className="font-['Noto_Sans_KR',sans-serif] text-[14px] md:text-[16px] text-foreground leading-[1.4]">
                {featured.body}
              </p>
            </div>
          </div>
      ...
      <section className="w-full bg-background">
        <div className="max-w-[1200px] mx-auto px-4 sm:px-8 md:px-16 py-10 sm:py-12 md:py-16 flex flex-col items-center gap-10 sm:gap-12">
          ...
          <div className="text-center">
            ...
            <p className="font-['Noto_Sans_KR',sans-serif] text-[16px] md:text-[20px] text-text-secondary leading-[1.2] mt-2">
              {activeTab === "notice"
                ? "IT노조의 주요 공지사항"
                : activeTab === "gallery"
                  ? "활동 사진 갤러리"
          ...
          <div className="flex flex-col gap-6 w-full">
            {filteredArticles.map((article) => (
              <div
                key={article.id}
                className="relative bg-background rounded-lg min-w-[240px] w-full cursor-pointer hover:shadow-md transition-shadow"
                ...
                <div className="flex flex-col sm:flex-row gap-6 p-6 items-start">
                  ...
                  <div className="flex flex-col gap-4 flex-1 min-w-[160px]">
                    <div className="flex flex-col gap-2">
                      ...
                      <p className="font-['Noto_Sans_KR',sans-serif] text-[14px] md:text-[16px] text-text-secondary leading-[1.4]">
                        {article.body}
                      </p>
                    </div>
                    <div className="flex gap-4 items-center">
                      <button className="bg-btn-dark text-[#f5f5f5] text-[16px] font-['Noto_Sans_KR',sans-serif] px-3 py-3 rounded-lg border border-btn-dark hover:bg-btn-dark-hover transition-colors">
                        자세히 보기
                      </button>
                    </div>
                  </div>
